...
{#if type === "file"}
  <input
    tabindex={restProps.readonly ? -1 : restProps.tabindex}
    bind:this={ref}
    data-slot={dataSlot}
    class={cn(
      ...
      "read-only:pointer-events-none read-only:selection:bg-transparent read-only:opacity-50 read-only:focus-visible:border-input read-only:focus-visible:ring-transparent",
      className,
    )}
    type="file"
    bind:files
